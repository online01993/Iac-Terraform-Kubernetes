...
resource "terraform_data" "cloud_init_finish_and_reboot" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      ssh -o StrictHostKeyChecking=no -i ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key -o ConnectTimeout=2 robot@${xenorchestra_vm.vm[each.value.id].ipv4_addresses[0]} '(sleep 5; sudo reboot)&'; sleep 5      
