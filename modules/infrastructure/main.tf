...
resource "terraform_data" "cloud_init_finish_and_reboot" {
  depends_on = [
    xenorchestra_vm.vm
  ]
  for_each = { 
...
resource "terraform_data" "get_device_path_workers" {
  depends_on = [
    xenorchestra_vm.vm,
    terraform_data.cloud_init_finish_and_reboot
  ]
  for_each = { 
    for i in toset([ 
      for index, i in range(0,var.xen_infra_settings.worker_vm_request.vm_settings.count) : {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      echo "${tls_private_key.terrafrom_generated_private_key.private_key_openssh}" > ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key
      chmod 600 ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key
      ssh robot@${xenorchestra_vm.vm[each.value.id].ipv4_addresses[0]} -o StrictHostKeyChecking=no -i ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key sudo fdisk -l | grep ${var.xen_infra_settings.node_storage_request.storage.ssd.volume} | awk '{print $2}' | tr -d : | tr -d '\n' > ${path.module}/scripts/get_ssd_device_path_worker_${each.value.id}
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      echo "${tls_private_key.terrafrom_generated_private_key.private_key_openssh}" > ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key
      chmod 600 ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key
      ssh robot@${xenorchestra_vm.vm[each.value.id].ipv4_addresses[0]} -o StrictHostKeyChecking=no -i ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key sudo fdisk -l | grep ${var.xen_infra_settings.node_storage_request.storage.nvme.volume} | awk '{print $2}' | tr -d : | tr -d '\n' > ${path.module}/scripts/get_nvme_device_path_worker_${each.value.id}
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      echo "${tls_private_key.terrafrom_generated_private_key.private_key_openssh}" > ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key
      chmod 600 ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key
      ssh robot@${xenorchestra_vm.vm[each.value.id].ipv4_addresses[0]} -o StrictHostKeyChecking=no -i ${path.module}/scripts/.robot_id_rsa_worker_${each.value.id}.key sudo fdisk -l | grep ${var.xen_infra_settings.node_storage_request.storage.hdd.volume} | awk '{print $2}' | tr -d : | tr -d '\n' > ${path.module}/scripts/get_hdd_device_path_worker_${each.value.id}
