  ...
  value = [
    for i in range(length(var.nodes)) : 
    {
      ...
      "ssd"   = var.nodes[i].storage.ssd.present ? ({
        ...
        "storage_class_name" = kubernetes_storage_class.storage_class_ssd_storage_replicated[count.index].metadata.name
        "storage_class_reclaim_policy" = kubernetes_storage_class.storage_class_ssd_storage_replicated[count.index].reclaim_policy
        "storage_class_storage_provisioner" = kubernetes_storage_class.storage_class_ssd_storage_replicated[count.index].storage_provisioner
        "storage_class_volume_binding_mode" = kubernetes_storage_class.storage_class_ssd_storage_replicated[count.index].volume_binding_mode
      }) : null
