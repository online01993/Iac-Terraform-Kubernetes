...
locals {
  ...
  crds_valid_yaml = [for index, doc in local.crds_split_doc : doc if try(yamldecode(doc).metadata.name, "") != ""]
  crds_dict       = { for index, doc in toset(local.crds_valid_yaml) : index(yamldecode(doc).metadata.name, doc) => doc }
}
