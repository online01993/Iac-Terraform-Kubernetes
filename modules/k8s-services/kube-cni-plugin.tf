...
# depends_on                    = [
 ...
 #yaml_body                     = each.value
 count      = length(fileset(data.kubectl_file_documents.k8s_cni_plugin_yaml_file.documents))
 yaml_body  = element(data.kubectl_file_documents.k8s_cni_plugin_yaml_file.documents, count.index)
}