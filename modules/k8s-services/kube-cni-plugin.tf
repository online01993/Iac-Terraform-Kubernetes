...
# depends_on                    = [
 ...
 for_each                      = toset({ for doc in toset([for doc in data.kubectl_path_documents.k8s_cni_plugin_yaml_file.documents : doc if try(yamldecode(doc).metadata.name, "") != ""]) : yamldecode(doc).metadata.name => doc })
 yaml_body                     = each.value
