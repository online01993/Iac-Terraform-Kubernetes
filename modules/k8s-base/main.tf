...
resource "terraform_data" "k8s-base-setup_01_resource_masters" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ssh -o StrictHostKeyChecking=no -i ./.robot_id_rsa_master_config_file"${each.value.id}".key -o ConnectTimeout=2 robot@${var.masters[0].address} '(sleep 2; sudo reboot)&'; sleep 5      
      until ssh -o StrictHostKeyChecking=no -i ./.robot_id_rsa_master_config_file"${each.value.id}".key -o ConnectTimeout=2 robot@${var.masters[0].address} true 2> /dev/null
      do
        echo "Waiting for OS to reboot and become available..."
        sleep 3
      done
      rm -rvf ./.robot_id_rsa_master_config_file"${each.value.id}".key
    EOT
  }
  provisioner "file" {
