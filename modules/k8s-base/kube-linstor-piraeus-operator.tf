...
/*resource "kubectl_manifest" "linstorcluster_all" {
 ...
 for_each = {
    for i in toset([
      for index, i in (split("---", templatefile("${path.module}/scripts/1.yaml", {
        })
      )) :
      {
        "id"  = index
